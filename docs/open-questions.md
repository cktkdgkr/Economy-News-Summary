...
| Q9 | LLM 공급자 후보 비교(OpenAI / Anthropic / Google / HyperCLOVA X): 한국어 키워드 요약 품질·비용·약관·지연 평가. | researcher → planner | RESOLVED | **Google Gemini 2.5 Flash로 변경(2026-05-22 사용자 결정)** — 무료 요구사항 충족을 위해 재선정. 1차 `gemini-2.5-flash`, 폴백 `gemini-2.0-flash`, AI Studio 무료 티어, 백엔드 프록시 경유. 근거: Q15 / [docs/research/q15-free-llm-providers.md](research/q15-free-llm-providers.md). (이전 결정: Anthropic Claude 고정 — 폐기.) |
...
| Q15 | **무조건 무료 LLM API 선정**: 카드 등록·자동 과금 없이 Google Play 상업용 앱 백엔드에서 쓸 수 있는 LLM API 중 최선의 한국어 요약 품질·한도 적합성 비교(Gemini/Groq/Cloudflare/OpenRouter/HF/Mistral/Cohere). | researcher | RESOLVED | **1순위: Gemini 2.5 Flash (AI Studio 무료)** — 카드 불필요, 1,500 RPD/10 RPM/250k TPM, 한국어 공식 최적화, 상업용 허용(한국 대상 앱 기준). **2순위: Groq Cloud (llama-3.3-70b-versatile)** — 카드 불필요, 1,000 RPD/30 RPM, 상업용 허용(Services Agreement), 단 한국어 미공식 지원. Mistral·Cohere 탈락(상업용 명시 불가). → [docs/research/q15-free-llm-providers.md](research/q15-free-llm-providers.md) |
